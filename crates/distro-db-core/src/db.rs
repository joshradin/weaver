...
use crate::storage_engine::{EngineKey, IN_MEMORY_KEY, storage_engine_factory, StorageEngineFactory, StorageError, Table};
...
impl DistroDb {
    pub fn new() -> Result<Self, Error> {
        let engines = EngineKey::all()
            .filter_map(|key| {
                match key.as_ref() {
                    IN_MEMORY_KEY => {
                        Some((key, storage_engine_factory(|schema: &TableSchema| Ok(Box::new(InMemory::new(schema.clone()))))))
                    }
                    _ => None
                }
            })
    ...
    pub fn open_table(&self, schema: &TableSchema) -> Result<(), Error> {
        ...
        let engine = self.engines.get(schema.engine()).ok_or_else(|| Error::CreateTableError)?;
    ...
    pub fn get_table(&self, schema: &str, name: &str) -> Option<Arc<Table>> {
        self.open_tables.read().get(&(schema.to_string(), name.to_string())).cloned()
    }
