...
impl QueryPlanFactory {
    ...
    fn to_plan_node(&self, query: &Query, db: &DbSocket) -> Result<QueryPlanNode, Error> {
        let node = match query {
            Query::Select { columns, table_ref, condition, limit, offset } => {
                let table= db.get_table(table_ref)?;
                let mut applicable_keys =
                    condition.as_ref().map(|condition| {
                        let condition = condition.columns();
                        table.schema()
                             .keys()
                             .iter()
                             .filter_map(|key| if key.columns().iter().all(|column| condition.contains(&column_reference(&table, column))) {
                                 Some(key)
                             } else {
                                 None
                             })
                ...
                applicable_keys.sort_by_cached_key(|key| if key.primary() { -1_isize} else { key.columns().len() as isize });
                let keys = applicable_keys.into_iter().map(|key| {
...
pub fn column_reference(table: &Table, column: &String) -> (String, String, String) {
    (table.schema().schema().to_string(), table.schema().name().to_string(), column.to_string())
}
