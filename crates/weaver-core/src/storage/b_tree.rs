...
use crate::storage::cells::{Cell, KeyValueCell};
...
impl DiskBTree {
    ...
    pub fn insert(&mut self, key_data: KeyData, row: OwnedRow) -> Result<(), Error> {
        ...
        // attempt 1:
        let split_id = match root.insert(key_data.clone(), &row, &self.flat_tree) {
    ...
    fn insert_ranged(
        ...
        for (range, child) in ranges {
            if range.contains(&key_data) {
                let mut child_node = nodes[*child as usize].write();
