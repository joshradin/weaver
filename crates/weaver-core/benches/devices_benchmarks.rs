...
fn init_device<T : StorageDevice + ?Sized>(device: &mut T) {
    device.set_len(STORAGE_DEVICE_LEN).expect("could not set length");
}
fn read_device<T : StorageDevice + ?Sized>(device: &mut T) {
    for _ in  0..IOPS {
        let at = rand::thread_rng().gen_range(0..STORAGE_DEVICE_LEN);
        let mut buffer = [0_u8; 8];
        device.read(at, &mut buffer).expect("could not read at most 8 bytes.");
    }
}
...
fn write_device<T : StorageDevice + ?Sized>(device: &mut T) {
    for _ in  0..IOPS {
        ...
        device.write(at, &buffer).expect("could not write at most 8 bytes.");
    }
}
...
fn bench_device<T : StorageDevice + ?Sized>(device: &mut T, group: &mut BenchmarkGroup<WallTime>) {
    ...
    group.bench_function("read throughput", |b| {
        b.iter(|| read_device(device))
    });
    group.bench_function("write throughput", |b| {
